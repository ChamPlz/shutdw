...
animação pulse), mantendo o botão "cancelar" e o "×".
...
- **Barra** (`.pill`): fundo escuro translúcido `rgba(15, 23, 42, 0.92)`
  (mais opaco que o toast, pois não há backdrop-blur sobre o desktop), borda
  `1px solid rgba(167, 139, 250, 0.4)`, `border-radius: 12px`, sombra roxa,
  padding `12px 16px`, animação `pulse` (opacidade 1 → 0.7).
...
- **Botão "×"** (`.close`): círculo pequeno no canto superior direito, fundo
  escuro translúcido + borda roxa, `×` branco, `no-drag`.
